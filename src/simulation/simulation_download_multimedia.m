    ...
    for i2 = 1:numEnter1
        for i3 = 1:numPackets1
            for i4 = 1:numInterarrival1
                ...
                for b = 1:numBps
                    simulator = dcf_simulator(pSuccess, 0.0, physical_type, physical_payload, physical_speed);
                    ...
                    fName = sprintf('./../results/simulation_download_multimedia-%d_%d_%d_%d_%d_%d.sim', pArrive1(i1), pEnter1(i2), nPackets1(i3), nInterarrival1(i4), bps(b), payload(p));
                    fprintf('\n------------\nrunning test: %s\n', fName);
                    fid = fopen(fName, 'w');
                    if (fid == -1)
        ...
        for i3 = 1:numPackets1
            for i4 = 1:numInterarrival1
                ...
                for b = 1:numBps
                    ...
                    fileName = sprintf('./../results/fig-simulation_download_multimedia-payload-%d_%d_%d_%d_%d.fig', pArrive1(i1), pEnter1(i2), nPackets1(i3), nInterarrival1(i4), bps(b));
                ...
                for p = 1:numPayload
                    ...
                    fileName = sprintf('./../results/fig-simulation_download_multimedia-bps-%d_%d_%d_%d_%d.fig', pArrive1(i1), pEnter1(i2), nPackets1(i3), nInterarrival1(i4), payload(p));
                    saveas(gcf,['.', filesep, fileName], 'fig');  % gca?
                end
                end
            end
